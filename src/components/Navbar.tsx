...
const navLinkBase =
  'px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-full text-sm sm:text-base transition-all duration-300 border border-transparent';
...
export default function Navbar() {
    ...
    languages.length === 2 ? (
      ...
      <div ref={langMenuRef} className="relative">
        <button
          ...
          className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-3 sm:py-2 rounded-full text-sm sm:text-base border border-white/10 bg-white/5 text-slate-100 hover:bg-white/10 transition"
          aria-label="Language"
          aria-haspopup="menu"
  ...
  return (
    <header id="site-header" className="sticky top-0 z-40 w-full border-b border-white/5 bg-[#0c1324]/70 backdrop-blur">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-2 sm:py-3 flex items-center justify-between gap-3 sm:gap-4">
        <Link
          to="/"
          className="text-white font-semibold tracking-tight text-base sm:text-lg md:text-xl relative"
        >
          {brand || 'Codefolio'}
